...
[width="100%",cols="10%,10%,10%a,10%,60%",options="header",]
|===
| Parameter name | Type | Optional | Default value | Info


| separator | String |  |  | Separator used to split a record in tokens (mandatory)

| allowEmptyStream | boolean | icon:check[] | false | The allowEmptyStream parameter will allow to prcoess the unavaiable stream for CSV file.

| autospanLine | boolean | icon:check[] | false | Last record spans rest of line (optional)

| crlf | String | icon:check[] | WINDOWS a| Character to be used to add a carriage return after each record (optional) Three values can be used : WINDOWS,
UNIX or MAC.

| endWithLineBreak | boolean | icon:check[] | true | The endWithLineBreak parameter flags if the CSV file should end with a line break or not (optional)

| generateHeaderColumns | boolean | icon:check[] | false | The generateHeaderColumns parameter allow to add in the CSV generated the header containing names of the columns

| isOrdered | boolean | icon:check[] | false | Indicates if the message must be ordered in output

| name | String | icon:check[] |  | Name describing the record (optional)

| quote | String | icon:check[] | " | Whether to marshal columns with the given quote character (optional)

| quoting | boolean | icon:check[] | false | Indicate if the values (and headers) must be quoted when marshaling (optional)

| quotingEscaped | boolean | icon:check[] | false | Indicate if the values must be escaped when quoting (optional)

| removeQuotes | boolean | icon:check[] | true | The remove quotes parameter flags if unmarshalling should try to remove quotes for each field

| skipField | boolean | icon:check[] | false a| The skipField parameter will allow to skip fields of a CSV file. If some fields are not necessary, they can be
skipped.

| skipFirstLine | boolean | icon:check[] | false a| The skipFirstLine parameter will allow to skip or not the first line of a CSV file. This line often contains
columns definition
...
| linkType | LinkType | icon:check[] | OneToOne | Type of link identifying the relation between the classes
